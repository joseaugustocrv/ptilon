# Ptilon

Software engineering with human and artificial intelligence working together.

Ptilon is a methodology for structured, traceable, and verifiable AI-assisted software engineering.

## Start here

- [Methodology](docs/methodology/README.md)
- [Workflow](docs/workflow/README.md)
- [Governance](docs/governance/README.md)
- [Security](docs/security/README.md)
- [Quality](docs/quality/README.md)
- [Standards](docs/standards/README.md)
- [Prompts](prompts/README.md)
- [Intent](docs/methodology/intent.md)
- [Intent Template](templates/intent.md)
- [Requirements Template](templates/requirements.md)
- [Examples](examples/README.md)
- [References](references/README.md)

## Repository structure

The Ptilon v1 content is organized under `v1/`:

- `docs/` — methodology and operational documentation
- `prompts/` — reusable AI-assisted engineering prompts
- `templates/` — reusable methodology artifacts
- `examples/` — complete worked examples
- `references/` — standards and engineering references
- `assets/` — visual identity and supporting assets

The repository root contains project-level governance, contribution guidance,
automation, licensing, and release history.
